...
const EditarSucursal: React.FC<EditarSucursalProps> = ({
  ...
  const activeCompany = useSelector(
    (state: RootState) => state.conectCompanyBranchSlice.activeCompany
  );

  const [selectedCountry, setSelectedCountry] = useState<number | null>(
    sucursalActual.domicilio.localidad.provincia.pais.id
  );
  const [selectedProvince, setSelectedProvince] = useState<number | null>(
    sucursalActual.domicilio.localidad.provincia.id
  );
  const [selectedLocality, setSelectedLocality] = useState<number | null>(
    sucursalActual.domicilio.localidad.id
  );
  console.log("Inicial", selectedLocality);
  ...
  useEffect(() => {
    const fetchProvinces = async () => {
      if (selectedCountry) {
        try {
          const provincesData =
            await provinciasService.getAllProvincesByCountry(selectedCountry);
    ...
    fetchProvinces();
  }, [selectedCountry, sucursalActual]);
  ...
  useEffect(() => {
      ...
      if (selectedProvince) {
        try {
          const localitiesData =
            await localidadesService.getAllLocalitiesByProvince(
              selectedProvince
            );
    ...
    fetchLocalities();
  }, [selectedProvince, sucursalActual]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sucursalActualizada = await branchServices.getById(
        initialValues.id
      );
